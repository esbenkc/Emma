...
class SentidaV2():
    ...
    def sentidaV2(self, text, output = ["mean", "total", "by_sentence_mean", "by_sentence_total"], normal = True, speed = ["normal", "fast"]):
        ...
        if speed  == "fast":
            ...
            senti_words = np.array([self.get_sentiment(re.sub('\/|\,|\'|\"', '', string.lower()).split()) for string in sents])
            if output == "mean":
                senti = np.sum([np.sum(words) / len(words) for words in senti_words]) / len(senti_words)
            elif output == "total":
                senti = np.sum(senti_words)
            ...
            if normal == True:
                senti = np.multiply([float(i) for i in senti], [0.2] * len(senti))
                senti = np.where(senti < -1, -1, np.where(senti > 1, 1, senti))
            return senti
